...
import nl.yoerivanhoek.rijksdemo.ui.ArtOverviewViewModel.ArtUIModel
import nl.yoerivanhoek.rijksdemo.ui.ArtOverviewViewModel.ArtUIModel.ArtData
        ...
        for (index in 0 until artItems.itemCount) {
            artItems.peek(index)?.let {
                when (it) {
                    ...
                    is ArtData -> item {
                        val artData = artItems[index] as ArtData
...
@Composable
fun ArtItem(modifier: Modifier = Modifier, artItem: ArtData) {
    Card(
        modifier = modifier
            .padding(16.dp)
    ) {
